...
class DDPGAgent:
    ...
    def save(self, filename):
        ...
        print(
            f"[AGENT] Pesos guardados como: {filename}_actor.pth y {filename}_critic.pth")

    def load(self, filename):
        """Carga pesos previamente entrenados"""
        if os.path.exists(f"{filename}_actor.pth"):
            self.actor.load_state_dict(torch.load(
                f"{filename}_actor.pth", map_location=self.device))
            self.critic.load_state_dict(torch.load(
                f"{filename}_critic.pth", map_location=self.device))
